...
**Preserved artifact.** The T0.1 fleet's `self-removal-abuse` agent wrote a debt-escape probe (`ZZProbeSelfRemovalAbuseTests.cs`) that it never ran before the session hit quota; the T0.1 fleet re-run executes it. Fold the confirmed version into this slice as the failing test to drive the fix.
